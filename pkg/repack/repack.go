...
func Repack(metadata string, output string, inputFolder string) {
	directories, err := readYAML(metadata)
	if err != nil {
		fmt.Println("Error reading YAML file:", err)
		return
	}

	// Check whether output file's folder exists
	outputFolder := filepath.Dir(output)
	if _, err := os.Stat(outputFolder); os.IsNotExist(err) {
		fmt.Println("Output folder does not exist:", outputFolder)
		// Create the folder
	...
	// BPK0
	// 30 AC 25 00
	// 目录的Offset （原为0x12242C10）
	// 10 34 05 00
	// 首个Entry的Offset （原为0x121EF800）
	...
	for dirIndex, directory := range directories {
		...
		for fileIndex, file := range directory.Files {
			...
			// 从对应的目录打开对应的文件
			fileContent, err := os.ReadFile(filepath.Join(inputFolder, directory.Name, file.Name))
			if err != nil {
